    ...
    def forward(self, x, jacobian=False, return_jac_type=False):
        ...
        if jacobian:
            ...
            if nobatch: 
                x = x.unsqueeze(0) 
            for module in self._modules.values():
                #print(type(module).__name__, 'x:', x.shape)
                x = module(x)
            if nobatch:
                x = x.squeeze(0)
...
class GraphConv(MessagePassing):
    ...
    def __init__(self, in_channels: Union[int, Tuple[int, int]],
                 out_channels: int, edge_index,batch_size=2, aggr: str = 'add', bias: bool = True,
                 **kwargs):
        ...
        edge_index_singular = edge_index[:, :edge_index.shape[1]//batch_size].to('cuda')
        self.num_vertices = torch.max(edge_index).item() + 1
        self.num_vertices_singular = self.num_vertices // batch_size
        self.edge_index_singular = edge_index_singular
        num_neighbours = torch.tensor([len(torch.where(edge_index_singular[0] == x)) for x in range(self.num_vertices//batch_size)])
        num_neighbours = num_neighbours[edge_index_singular[1]]
        self.num_neighbours = torch.stack(out_channels * [torch.stack(in_channels * [num_neighbours], 1)], 1).to('cuda')
    ...
    def _jacobian(self, x, val):
        J_analytical = torch.zeros(self.num_vertices_singular, self.out_channels, self.num_vertices_singular, self.in_channels).to('cuda')

        idx = torch.arange(self.num_vertices_singular).to('cuda')
        J_analytical[idx, :, idx,
        :] = self.lin_r.weight  # the *diagonal* is just a linear layer, so it's jacobian is it's weight
        J_analytical[self.edge_index_singular[0], :, self.edge_index_singular[1], :] = self.lin_l.weight / self.num_neighbours

        J = torch.reshape(J_analytical,
                          (1, self.num_vertices_singular * self.out_channels, self.num_vertices_singular * self.in_channels))

        return J, JacType.FULL

    def _jac_mul(self, x, val, Jseq, JseqType):
        J, _ = self._jacobian(x, val)  # (out)x(in) -- this is the Jacobian of the function
        J = J[0]
        # We either want to (matrix) multiply the current Jacobian with a
        #  *) vector: size (B)x(in)
        #       This should be interpreted as a diagonal matrix of size
    ...
    def forward(self, x: Union[Tensor, OptPairTensor],
                edge_weight: OptTensor = None, size: Size = None, jacobian=False):
        """"""
        batch_size = x.shape[0]
        x = torch.reshape(x, (-1, self.in_channels))
        # print("Before propagation:", x.shape)
        ...
        #print(x)
        if batch_size == 1:
            edge_index = self.edge_index_singular
        else:
            edge_index = self.edge_index
        out = self.propagate(edge_index, x=x, edge_weight=edge_weight,
                             size=size)
        # print("After propagation:", out.shape)
        #print(self.lin_l.weight.shape)
        #print(out.shape)
        out = self.lin_l(out)
        x_r = x[1]
        if x_r is not None:
...
class ResidualBlock(nn.Module):
    def __init__(self, *args, in_features=None, out_features=None):
        ...
        if len(args) is 1 and isinstance(args[0], (modules.container.Sequential, Sequential)): #исправил 19.10.23 было "is" поменял на "=="!!!
            self._F = args[0]
        else:
...
def fd_jacobian(function, x, h=1e-4):
    ...
    no_batch = x.dim() is 1 #исправил 19.10.23 было "is" поменял на "=="!!!
    if no_batch:
        x = x.unsqueeze(0)
    elif x.dim() > 2:
